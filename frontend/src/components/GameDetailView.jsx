...
function CollectiblePreview({ item }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="w-12 h-12 rounded-xl bg-[var(--color-nature-canvas-bg)] border border-[var(--color-forest)]/10 flex items-center justify-center overflow-hidden">
        {showFallbackIcon ? (
  ...
  return (
    <div className="flex flex-col items-center h-full p-6 overflow-y-auto">
      ...
      <div className="w-full max-w-lg mb-4">
        <button
          onClick={onBack}
          className="text-sm text-[var(--color-forest)] hover:text-[var(--color-forest-dark)] font-medium transition-colors cursor-pointer"
        >
          &larr; Back to games
        </button>
      </div>
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-lg mb-3 ring-2 ring-[var(--color-forest)]/20">
          <img src={photo.src} alt={photo.label} className="w-full h-full object-cover" />
        </div>
        <h2 className="text-2xl font-bold font-display text-[var(--color-forest-dark)] tracking-tight mb-2">
          {photo.label}
        </h2>
      ...
      {/* About This Activity */}
      <div className="w-full max-w-lg rounded-2xl bg-white/70 border border-[var(--color-forest)]/10 p-4 mb-4 shadow-sm">
        <h3 className="text-sm font-bold text-[var(--color-forest-dark)] mb-2">About This Activity</h3>
        {s.metaphor && (
          <p className="text-sm text-gray-600 italic mb-3">&ldquo;{s.metaphor}&rdquo;</p>
        )}
        {s.role_title && (
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white/70 border border-[var(--color-forest)]/10 p-4 mb-6 shadow-sm">
              ...
              <ol className="space-y-1.5">
                {s.round_scenarios.map((scenario, i) => (
                  <li key={i} className="flex items-start gap-2.5">
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[var(--color-forest)]/10 text-[var(--color-forest-dark)] text-[11px] font-bold flex items-center justify-center mt-0.5">
                      {i + 1}
                    </span>
                    <span className="text-sm text-gray-600 capitalize">{scenario}</span>
                  </li>
                ))}
              </ol>
            )}
      ...
      <div className="flex items-center gap-2 w-full max-w-lg mb-4">
        ...
        <LeafIcon className="w-4 h-4 text-[var(--color-forest)]/25" />
        <div className="flex-1 h-px bg-[var(--color-forest)]/15" />
      </div>
      ...
      <button
        onClick={onStart}
        disabled={isLoading}
        className="w-full max-w-lg py-3 rounded-2xl font-bold text-white text-sm bg-gradient-to-r from-[var(--color-forest)] to-[var(--color-forest-dark)] shadow-lg hover:shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
      >
        {isLoading ? (
