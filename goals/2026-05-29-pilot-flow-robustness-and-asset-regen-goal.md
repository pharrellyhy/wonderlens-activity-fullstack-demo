...
Run in Claude Code goal mode (requires v2.1.139+), or Codex `/goal`. The command stays short on purpose: this file's **Required Checks** and **Final Completion Gate** are authoritative, and the worker reads the file — the `/goal` line only needs to point at the file, inline a compact end state for the evaluator (which reads the transcript, not the file), and bound the run. The autonomous loop targets Streams 1+2 and 4 plus Stream 3 scaffolding; image art stays human-gated.

```text
/goal Run goals/2026-05-29-pilot-flow-robustness-and-asset-regen-goal.md — read it fully and obey its Hard Constraints, Mandatory Ordering, and human-gated Stream 3 art rule. Done when its Final Completion Gate holds: every Required Check (backend pytest suites + asset-contract + full suite + ruff; frontend test/lint/build; git diff --check; and the live-provider smoke with credentials sourced) passes with its output shown in this conversation, and Stream 3 image art is left for human sign-off. Or stop after 60 turns and report the blocker.
```
