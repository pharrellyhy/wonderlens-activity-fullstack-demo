export default function RetryButton({ onRetry, retryCount = 0, maxRetries = 3 }) {
  if (retryCount >= maxRetries) {
    return (
      <div className="flex flex-col items-center gap-2 p-4">
        <div className="text-sm text-amber-600 bg-amber-50/80 px-4 py-2 rounded-full border border-amber-200/50">
          Using backup mode
        </div>
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-6">
      ...
      <button
        onClick={onRetry}
        className="px-6 py-2.5 bg-gray-800 text-white rounded-full hover:bg-gray-700 transition-all font-semibold shadow-sm hover:shadow-md"
      >
        Let&apos;s try again!
      </button>
      {retryCount > 0 && (
