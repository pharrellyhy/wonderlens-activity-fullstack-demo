export default function PhotoGrid({ photos = [], animation }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-4">
      <h3 className="text-lg font-bold text-purple-700">Your Collection</h3>
      ...
      <div className={`grid grid-cols-2 gap-3 ${
        ...
        {slots.map((photo, i) => (
          <div
            key={i}
            className="w-28 h-28 rounded-xl overflow-hidden border-2 border-white shadow-md bg-gray-50 flex items-center justify-center"
          >
            {photo ? (
      ...
      {animation === 'connection_lines_draw' && (
        <p className="text-sm text-purple-600 font-medium animate-bounce">Connected!</p>
      )}
    </div>
  );
}
