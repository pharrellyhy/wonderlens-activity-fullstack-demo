...
One new field added to `SessionStateModel`:
- `synthesis_story_quality: str` — stores the last classification quality ("good", "weak", or ""), set during synthesis evaluate/improve phases
...
| File | Change |
|------|--------|
| `backend/turn_handler.py` | Add `_build_phase_timeline()`, call from `_build_debug_payload()`, set `synthesis_story_quality` |
| `backend/schemas/session_state.py` | Add `synthesis_story_quality` field |
