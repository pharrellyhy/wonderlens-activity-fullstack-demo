...
- `cd frontend && npx eslint src/hooks/useConversation.js src/hooks/useSessionOrchestration.js src/hooks/useCharacterSfx.js` - blocked in this worktree because local frontend dependencies are incomplete (`@eslint/js` missing from resolution), so frontend verification here is via focused source regressions plus code review rather than ESLint
