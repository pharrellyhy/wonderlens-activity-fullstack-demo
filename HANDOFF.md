...
**Verification** (worktree; live provider, creds sourced, local proxy bypassed via `NO_PROXY`): Streams-1+2 suite **49 passed**; asset-contract **6 passed, 1 pre-existing carousel-vs-picker red (allowed)**; backend ruff `turn_handling/ agents/script_agent.py` **clean**; frontend **72 tests / lint / build green**; `git diff --check` **clean**; live smoke **11/12** (career missed a source term on one run, **passed on re-run** — live phrasing variability); full backend suite **112 passed, 2 skipped, 4 failed** — the 4 are live-LLM T0 quality assertions in `test_ai_quality` on **non-pilot** activities (`test_t0_cat1_full_flow[dog/cat/dinosaur]` open-question-without-scaffold + 1 flaky `silence_no_specific_items` that passes on retry); these paths are untouched by this work and are not regressions. **Blocker to a 100%-green full suite**: pre-existing live-LLM quality flakiness in `test_ai_quality` for non-pilot activities (out of scope). **Human-gated remainder**: Stream 3 image-art generation + per-pilot sign-off, and the Cat1/Cat3/Cat5 browser walkthrough at `/?view=activities`.
